...
def create_account(request):
        if request.method == 'GET':
            page = 'templates/create_account.html'
            joint_acc = ['select','yes','no']        
            account_type = ['select','saving','current','credit'] 
            gender = ['select','Male' , 'Female' , 'Transgender'] 
            return render(request,page,{'joint_acc':joint_acc,
        ...
        if request.method == 'POST':               
            kwargs = {}                        
            kwargs['account_name'] = request.POST.get('account_name')                                                  
            kwargs['balance'] = request.POST.get('balance')
            kwargs['account_type'] = request.POST.get('account_type')
            kwargs['joint_acc'] = request.POST.get('joint_account')

            kwargs['address'] = request.POST.get('address')
            kwargs['phone_no'] = request.POST.get('phone')
            kwargs['email'] = request.POST.get('email')

            kwargs['name'] = request.POST.get('name')
            kwargs['age'] = request.POST.get('age')
            kwargs['gender'] = request.POST.get('gender')
            Database()
            Database.save_into_database(kwargs)
...
class Database:    
    ...
    def save_into_database(kwargs):                
        try:
            conn = sqlite3.connect("db.sqlite3")     # connect to database              
